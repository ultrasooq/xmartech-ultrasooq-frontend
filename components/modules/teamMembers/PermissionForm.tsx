...
import { usePermissions,useSetPermission, useGetPermission, useUpdatePermission } from "@/apis/queries/member.queries";
...
const PermissionForm: React.FC<PermissionFormProps> = ({ roleId, onClosePer }) => {
  ...
  const onSubmit = async (formData: any) => {
    ...
    if (memoizedGetPermission) {
       response = await updatePermission.mutateAsync({ userRoleId: roleId, ...formData });
    } else {
      response = await addPermission.mutateAsync({ userRoleId: roleId, ...formData });
    }
          ...
          >
            <div className="text-normal m-auto mb-7 w-full text-center text-sm leading-6 text-light-gray">
              <h2 className="mb-3 text-center text-3xl font-semibold leading-8 text-color-dark sm:text-4xl sm:leading-10">
                { memoizedGetPermission.length ? "Update Permission" : "Add Permission" }
              </h2>
            </div>
            ...
            <div>
              <div className="mb-3.5 w-full">
                <AccordionMultiSelectV2
                  ...
                  error={form.formState.errors?.permissionIdList?.message !== undefined ? String(form.formState.errors?.permissionIdList?.message) : ""}
                />
              </div>
